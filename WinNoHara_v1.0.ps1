...
function Collect {
 ...
 $raw+="=== IPv6 Prefix Policy ===";try{$raw+=(Get-IPv6PrefixPolicyText)}catch{}
...
function Get-IPv6PrefixPolicyText {
 $lines = @(& netsh interface ipv6 show prefixpolicies 2>&1)
 $rows = foreach($line in $lines) {
  $t = [string]$line
  if($t -match '^\s*(\d+)\s+(\d+)\s+(\S+)\s*$') {
   [PSCustomObject]@{ Priority=[int]$matches[1]; Label=[int]$matches[2]; Prefix=$matches[3] }
  }
 }
 if(-not $rows) { return "IPv6プレフィックスポリシーを取得できませんでした。" }
 $out = @('優先順位  ラベル  プレフィックス','--------  ------  ------------------------------')
 foreach($r in $rows) { $out += ('{0,8}  {1,6}  {2}' -f $r.Priority,$r.Label,$r.Prefix) }
 return ($out -join "`r`n")
}
 ...
 try {
  $script:CurrentJob = Start-Job -ArgumentList $commandLine -ScriptBlock {
   ...
   if($cmd -eq "netsh interface ipv6 show prefixpolicies") {
    ...
    if(-not $rows) { "IPv6プレフィックスポリシーを取得できませんでした。" }
    else {
     '優先順位  ラベル  プレフィックス'
     '--------  ------  ------------------------------'
     foreach($r in $rows) { '{0,8}  {1,6}  {2}' -f $r.Priority,$r.Label,$r.Prefix }
    }
   } else {
